    ...
    try {
      ...
      if (searchInput && !request.yar.get('hasSpecialCharacter')) {
        ...
        if (!Array.isArray(locationdetails) || locationdetails.length === 0) {
          ...
          if (result === null) {
            logger.error('OS Names API returned null')
            throw new Error('OS Names API failed')
          }
        ...
        if (searchValue != null && searchValue !== '') {
          ...
          if (MonitoringstResult === null) {
            logger.error('Monitoring Station API returned null')
            throw new Error('Monitoring Station API failed')
          }
          ...
          const hasStations =
            MonitoringstResult?.getmonitoringstation?.length > 0
