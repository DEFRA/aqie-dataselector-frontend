...
describe('customdatasetController', () => {
  ...
  // ─── last7days datasource override branches ─────────────────────────────────────

  describe('last7days datasource override branches', () => {
    it('renders other-only + last7days error view (with location selected)', async () => {
      mockRequest.yar.get.mockImplementation((key) => {
        const values = {
          ...
          selectedlocation: ['England'],
          datasourceGroups: [
            { category: 'Other data from Defra', networks: [{ id: 'a' }] }
          ]
        }
        return values[key]
      })
    ...
    it('does not show other-only error when no location selected', async () => {
      mockRequest.yar.get.mockImplementation((key) => {
        const values = {
          TimeSelectionMode: 'last7days',
          selectedyear: 'Last 7 days',
    ...
    it('sets downloadDatasourceGroups to near-realtime-only when both categories + last7days', async () => {
      mockRequest.yar.get.mockImplementation((key) => {
        const values = {
          TimeSelectionMode: 'last7days',
          selectedyear: 'Last 7 days',
          ...
          datasourceGroups: [
            ...
            { category: 'Other data from Defra', networks: [{ id: 2 }] }
          ]
        }
        return values[key]
      })
      ...
      expect(mockRequest.yar.set).toHaveBeenCalledWith(
        ...
        [{ category: 'Near real-time data from Defra', networks: [{ id: 1 }] }]
      )
      expect(mockRequest.yar.set).toHaveBeenCalledWith(
    ...
    it('sets downloadDatasourceGroups to full groups when NOT both+last7days', async () => {
      mockRequest.yar.get.mockImplementation((key) => {
        const values = {
          TimeSelectionMode: '',
          selectedyear: '2024',
    ...
    it('shows last7days warning banner for both categories on normal render path', async () => {
      mockRequest.yar.get.mockImplementation((key) => {
        const values = {
          TimeSelectionMode: 'last7days',
          selectedyear: 'Last 7 days',
    ...
    it('detects last7days via selectedyear text even when TimeSelectionMode is unset', async () => {
      mockRequest.yar.get.mockImplementation((key) => {
        const values = {
          TimeSelectionMode: '',
          selectedyear: 'last 7 days',
