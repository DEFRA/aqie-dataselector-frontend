...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Debounce timer: delay suggestion box by 3 seconds after the user stops typing.
  let debounceTimer = null

  accessibleAutocomplete({
    element: container,
    id: 'my-autocomplete',
    source: pollutantSource,
    minLength: 1,
    confirmOnBlur: false
  })

  // accessible-autocomplete opens the dropdown immediately on any keypress.
  // To delay it, we hide the menu via CSS on every input event and only
  // reveal it after the user has stopped typing for 3 seconds.
  const input = document.getElementById('my-autocomplete')
  const getMenu = () => container.querySelector('.autocomplete__menu')
